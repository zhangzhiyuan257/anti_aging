...
##### 数据读取 #####
##################
data_positive = readFasta("../data/positive_0.9.fasta")
# DDE_feature = DDE(data_positive)
CKSAAP_feature = CKSAAP(data_positive)
# CTriad_feature = CTriad(data_positive)

####################
##### 数据处理 ######
###################
# DDE_feature = DDE_feature[1:]
CKSAAP_feature = CKSAAP_feature[1:]
# CTriad_feature = CTriad_feature[1:]
# data_list_dde = [i[2:] for i in DDE_feature]
# data_list_cks = [i[2:] for i in CKSAAP_feature]
# data_list_ct = [i[2:] for i in CTriad_feature]
# data_all = [data_list_dde[i] + data_list_cks[i] + data_list_ct[i]  for i in range(len(data_list_dde))]
data_all = [i[2:] for i in CKSAAP_feature]
...
##### 阳性样本数据保存 #####
#########################
np.savetxt('../result/result_GAN/data_positive.txt',data_all)
...
plt.legend()
plt.grid()
plt.savefig("../result/training_loss_curve.png")  # 保存图像
plt.show()
